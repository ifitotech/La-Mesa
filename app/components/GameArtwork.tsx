...
export default function GameArtwork({ gameId, className = "", english = false }: { gameId: GameId; className?: string; english?: boolean }) {
  ...
  return <div className={`relative flex min-h-32 w-[38%] max-w-36 shrink-0 flex-col items-center justify-center overflow-hidden border-r border-amber-100/15 bg-gradient-to-br ${item.colors} ${className}`}>{item.image && <Image src={item.image} alt="" fill className="object-cover opacity-90 transition duration-500 group-hover:scale-105" />}<span className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,224,146,.18),transparent_42%),linear-gradient(to_right,rgba(2,8,5,.05),rgba(2,8,5,.58))]" /><span className="absolute inset-2 rounded-xl border border-white/10" /><span className="absolute -right-5 -top-7 text-9xl opacity-20 transition duration-500 group-hover:rotate-6 group-hover:scale-110">{item.emoji}</span><span className="relative text-5xl drop-shadow-lg">{item.emoji}</span><span className="relative mt-2 text-[10px] font-black uppercase tracking-[.18em] text-amber-50/90">{english ? englishLabels[gameId] ?? item.label : item.label}</span></div>;
}
