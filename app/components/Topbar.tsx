...
export default function Topbar() {
  ...
  return <header className="sticky top-0 z-40 flex min-h-16 items-center justify-between gap-3 border-b border-blue-200/10 bg-[#07111fe8] px-4 py-3 backdrop-blur-xl sm:px-6 xl:min-h-20 xl:px-8">
    <div className="flex min-w-0 items-center gap-3"><Link href="/dashboard" className="flex shrink-0 items-center gap-2"><Image src="/la-mesa-logo-v2.png" alt="Logo oficial de La Mesa" width={42} height={42} priority className="h-10 w-10 rounded-xl border border-blue-200/40 object-cover shadow-lg shadow-blue-950/70" /><span className="hidden text-lg font-black tracking-tight text-slate-100 sm:inline">LA MESA</span></Link><div className="min-w-0 border-l border-slate-700/70 pl-3"><p className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-[.16em] text-blue-300 xl:hidden"><Sparkles size={11} /> {title}</p><p className="hidden text-xs font-bold uppercase tracking-[.2em] text-violet-300 xl:flex"><Sparkles size={14} /> La Mesa</p><h1 className="hidden text-xl font-black xl:mt-1 xl:block">{title}</h1></div></div>
