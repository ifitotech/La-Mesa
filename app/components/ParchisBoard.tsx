  ...
  return (
    <div className="game-3d-board mx-auto aspect-square w-full max-w-[44rem] rounded-[1.6rem] border-[8px] border-[#6c4422] bg-[#eadfbf] p-1 shadow-[inset_0_0_0_2px_#d5a84d,0_8px_0_#3d2513,0_28px_55px_rgba(0,0,0,.55)]">
      <div className="relative grid h-full w-full grid-cols-[repeat(19,minmax(0,1fr))] grid-rows-[repeat(19,minmax(0,1fr))] overflow-hidden rounded-xl">
        <div className="parchis-home parchis-home-blue col-span-8 row-span-8 p-[14%]"><div className="h-full rounded-3xl shadow-inner" /></div>
        <div className="parchis-home parchis-home-yellow col-start-12 col-span-8 row-span-8 p-[14%]"><div className="h-full rounded-3xl shadow-inner" /></div>
        <div className="parchis-home parchis-home-green col-start-12 col-span-8 row-start-12 row-span-8 p-[14%]"><div className="h-full rounded-3xl shadow-inner" /></div>
        <div className="parchis-home parchis-home-red col-span-8 row-start-12 row-span-8 p-[14%]"><div className="h-full rounded-3xl shadow-inner" /></div>
        ...
        {Array.from({ length: 7 }, (_, index) => index + 1).flatMap((step) => [
          <span key={`b-${step}`} style={cellStyle({ x: 9, y: step })} className="parchis-lane parchis-lane-blue" />,
          <span key={`y-${step}`} style={cellStyle({ x: 18 - step, y: 9 })} className="parchis-lane parchis-lane-yellow" />,
          <span key={`g-${step}`} style={cellStyle({ x: 9, y: 18 - step })} className="parchis-lane parchis-lane-green" />,
          <span key={`r-${step}`} style={cellStyle({ x: step, y: 9 })} className="parchis-lane parchis-lane-red" />,
        ])}
