...
export default function DashboardPage() {
  ...
  return <AppLayout><div className="mx-auto max-w-6xl space-y-5 pb-4 md:space-y-7">
    <section className="relative overflow-hidden rounded-[2rem] border border-blue-200/20 bg-[linear-gradient(130deg,#06142e,#123574_58%,#08152e)] p-5 shadow-2xl shadow-blue-950/40 md:p-8">
      ...
      <div className="relative flex items-start justify-between gap-4"><div className="min-w-0"><p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.24em] text-amber-300"><Sparkles size={13} /> La Mesa · {country.name}</p><h1 className="mt-2 max-w-2xl text-[2rem] font-black leading-[1.08] sm:mt-3 sm:text-4xl md:text-5xl">{welcome}</h1><p className="mt-3 max-w-xl text-sm leading-6 text-blue-100/80 md:text-base">{isEnglish ? "Your social game host for sharing a great night with friends and family." : "Tu anfitrión para jugar, competir y pasar una noche increíble con los tuyos."}</p></div><Link href="/profile" aria-label={isEnglish ? "Open profile" : "Abrir perfil"} className="hidden shrink-0 rounded-2xl border border-white/15 bg-slate-950/30 p-1.5 shadow-lg sm:block"><Image src={`/avatars/${avatar}.png`} alt="" width={58} height={58} className="h-14 w-14 rounded-xl object-cover" /></Link></div>
    ...
    <section><div className="mb-3 flex items-end justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.24em] text-blue-300">{isEnglish ? "Games" : "Juegos"}</p><h2 className="mt-1 text-2xl font-black">{isEnglish ? "Pick a table" : "Elige una mesa"}</h2></div><Link href="/games" className="text-sm font-black text-blue-300 hover:text-white">{isEnglish ? "See all" : "Ver todos"}</Link></div><div className="grid gap-3 md:grid-cols-3">{games.map((game) => { const body = <><div className="absolute -right-5 -top-7 text-8xl opacity-25">{game.visual}</div><div className="relative flex items-start justify-between"><span className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/25 bg-black/15 text-xl sm:h-11 sm:w-11 sm:text-2xl">{game.icon}</span><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${game.ready ? "bg-blue-400/20 text-blue-100" : "bg-amber-300/20 text-amber-100"}`}>{game.ready ? (isEnglish ? "PLAY" : "JUGAR") : (isEnglish ? "SOON" : "PRONTO")}</span></div><div className="relative mt-4 sm:mt-6"><h3 className="text-xl font-black leading-none sm:text-2xl">{game.title}</h3><p className="mt-2 text-sm text-white/75 sm:min-h-10">{game.subtitle}</p><span className="mt-3 flex items-center gap-1 text-sm font-black sm:mt-4">{game.ready ? (isEnglish ? "Play now" : "Jugar ahora") : (isEnglish ? "In development" : "En desarrollo")} {game.ready && <ArrowRight size={16} />}</span></div></>; return game.ready && game.href ? <Link key={game.title} href={game.href} className={`relative overflow-hidden rounded-2xl border border-white/20 bg-gradient-to-br p-4 shadow-xl shadow-black/25 transition hover:-translate-y-1 hover:border-white/50 sm:p-5 ${game.accent}`}>{body}</Link> : <article key={game.title} className={`relative overflow-hidden rounded-2xl border border-white/15 bg-gradient-to-br p-4 opacity-85 sm:p-5 ${game.accent}`}>{body}</article>; })}</div></section>
