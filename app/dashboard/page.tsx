...
export default function DashboardPage() {
  ...
  const english = country.language === "en";
  const dashboardActions = english ? [
    { title: "Game Night", detail: "Play together in the same room", href: "/game-night", icon: Gamepad2, tone: "from-emerald-500 via-emerald-700 to-[#063d32]" },
    { title: "Online match", detail: "Create a room and share the code", href: "/online", icon: Radio, tone: "from-blue-500 via-blue-700 to-[#071d50]" },
    { title: "Trivia", detail: "U.S. culture, history and more", href: "/play/trivia", icon: Brain, tone: "from-violet-500 via-indigo-700 to-[#24124d]" },
  ] : actions;
  return <AppLayout><div className="mx-auto max-w-7xl space-y-6 md:space-y-8">
    <section className="relative overflow-hidden rounded-3xl border border-blue-200/20 bg-[linear-gradient(130deg,#071733,#102d68_58%,#07142c)] p-6 shadow-2xl shadow-blue-950/30 md:p-9"><div className="absolute -right-14 -top-20 h-64 w-64 rounded-full bg-blue-400/20 blur-3xl" /><div className="absolute -bottom-20 left-1/3 h-48 w-48 rounded-full bg-violet-500/15 blur-3xl" /><div className="relative max-w-2xl"><p className="text-xs font-bold uppercase tracking-[.3em] text-amber-300">La Mesa · {country.name}</p><h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{country.greeting}<br />{english ? "Let the night begin." : "Que empiece la reunión."}</h2><p className="mt-4 max-w-xl leading-7 text-slate-300">{english ? "Your host for playing, competing and connecting with family and friends, wherever they are." : "Tu anfitrión para jugar, competir y conectar con familia y amigos, estén donde estén."}</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/game-night" className="mesa-action inline-flex items-center gap-2"><Gamepad2 size={18} /> {english ? "Play now" : "Jugar ahora"}</Link><Link href="/online" className="rounded-xl border border-white/15 bg-slate-950/35 px-5 py-3 font-bold text-white transition hover:border-blue-200/60 hover:bg-blue-500/10">{english ? "Play online" : "Jugar online"}</Link></div></div></section>
    <section><div className="mb-4 flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-blue-300">{english ? "Your games" : "Tus juegos"}</p><h2 className="mt-1 text-2xl font-black">{english ? "What are we playing today?" : "¿Qué jugamos hoy?"}</h2></div><Trophy className="text-amber-300" /></div><div className="grid gap-3 md:grid-cols-3">{dashboardActions.map((action) => { const Icon = action.icon; return <Link key={action.title} href={action.href} className={`mesa-game-button bg-gradient-to-br ${action.tone}`}><span className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/20 bg-black/15"><Icon size={22} /></span><h3 className="mt-5 text-xl font-black">{action.title}</h3><p className="mt-1 text-sm text-blue-100/80">{action.detail}</p><span className="mt-5 flex items-center gap-1 text-sm font-bold">{english ? "Open" : "Abrir"} <ArrowRight size={16} /></span></Link>; })}</div></section>
