...
export default function HostScreenPage() {
  ...
  return <AppLayout lockViewport><div className="mx-auto flex min-h-full max-w-6xl flex-col gap-5 pb-3">
    ...
    {(session.history?.length ?? 0) > 0 && <section className="mesa-panel rounded-2xl p-4"><p className="text-xs font-black uppercase tracking-[.22em] text-violet-300">Historial de la noche</p><div className="mt-3 flex flex-wrap gap-2">{[...(session.history ?? [])].reverse().slice(0, 4).map((round) => <span key={round.id} className="rounded-xl border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm font-bold">{round.game}</span>)}</div></section>}
