...
import { ArrowLeft, Check, Hand, Lightbulb, RotateCcw, SkipForward } from "lucide-react";
...
export default function HeadsUpPage() {
  ...
  function next(correct: boolean) {
    if (correct) setPoints((value) => value + 1);
  ...
  return <AppLayout><div className="mx-auto max-w-3xl space-y-5">
    ...
    <section className="mesa-panel-gold rounded-3xl p-6 text-center md:p-8">
      <p className="text-xs font-bold uppercase tracking-[.24em] text-orange-300">Para 2 o más · {country.flag} {country.name}</p>
      <h1 className="mt-2 flex justify-center gap-3 text-3xl font-black"><Hand className="text-orange-300" /> Heads Up</h1>
      <p className="mx-auto mt-3 max-w-xl text-slate-300">Una persona sostiene la pantalla sin mirar la palabra; los demás dan pistas. Pulsa correcto o pasar.</p>
    </section>
    <section className="mesa-panel rounded-3xl p-7 text-center md:p-10">
      <div className="flex items-center justify-between text-sm font-bold text-slate-400"><span>{points} puntos</span><span className={seconds <= 10 ? "text-rose-300" : "text-amber-200"}>{seconds}s</span><span>{index + 1}/{words.length}</span></div>
      {playing ? <><h2 className="mt-10 text-5xl font-black md:text-6xl">{current.word}</h2><div className="mt-6 flex items-center justify-center gap-2 text-slate-400"><Lightbulb size={18} /> {current.hint}</div><div className="mt-10 flex flex-col justify-center gap-3 sm:flex-row"><button onClick={() => next(false)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-600 px-6 py-3 font-bold hover:bg-slate-800"><SkipForward size={18} /> Pasar</button><button onClick={() => next(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-3 font-bold"><Check size={18} /> Correcto</button></div></> : <div className="py-10"><h2 className="text-3xl font-black">{seconds === 0 ? "¡Tiempo terminado!" : "¿Listos para las pistas?"}</h2><p className="mt-3 text-slate-400">Consiguieron {points} {points === 1 ? "palabra" : "palabras"}.</p><button onClick={startNewRound} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-orange-500 to-rose-600 px-6 py-3 font-black"><RotateCcw size={18} /> {points ? "Nueva ronda" : "Empezar"}</button></div>}
    </section>
  </div></AppLayout>;
}
