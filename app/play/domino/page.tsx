...
export default function SoloDominoPage() {
  ...
  return <AppLayout lockViewport><div className="mx-auto max-w-5xl space-y-5">
    ...
    <section className="mesa-panel-gold rounded-3xl p-4 md:p-7">
      ...
      <div className="flex min-h-[280px] flex-wrap content-center justify-center gap-1 rounded-[2rem] border-[10px] border-[#5c3517] bg-[radial-gradient(circle_at_center,#17653b,#0a321d)] p-4 shadow-[inset_0_0_0_3px_#b17a3c,inset_0_0_40px_#031b0d]">{game.board.length === 0 ? <p className="text-sm font-bold text-emerald-100/70">{text.ready}</p> : game.board.map((tile) => <DominoTile key={tile.id} tile={tile} />)}</div>
