...
export default function SoloDominoPage() {
  ...
  return <AppLayout lockViewport><div className="mx-auto max-w-5xl space-y-5">
    ...
    <section className="domino-table-scene rounded-[2rem] p-4 pt-36 md:p-7 md:pt-44">
      ...
      <div className="flex min-h-[280px] flex-wrap content-center justify-center gap-1 rounded-[2rem] border-[8px] border-[#7b5123] bg-[radial-gradient(circle_at_center,#1e6446,#0b2d20_68%,#071c14)] p-4 shadow-[inset_0_0_0_2px_#d2a752,inset_0_0_55px_#031b0d,0_18px_42px_rgba(0,0,0,.35)]">{game.board.length === 0 ? <p className="text-sm font-bold uppercase tracking-[.18em] text-emerald-100/60">{text.ready}</p> : game.board.map((tile) => <DominoTile key={tile.id} tile={tile} />)}</div>
