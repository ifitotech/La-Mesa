...
export default function SoloGamesPage() {
  ...
  if (!trivia) {
    return (
      <AppLayout>
        <section className="mesa-panel mx-auto flex min-h-72 max-w-3xl flex-col items-center justify-center rounded-3xl p-8 text-center">
          <Bot className="text-violet-300" size={36} />
          <h1 className="mt-5 text-2xl font-black">
            {isEnglish ? "Solo mode is being prepared" : "Estamos preparando el modo individual"}
          </h1>
          <p className="mt-2 max-w-md text-sm leading-6 text-slate-400">
            {isEnglish
              ? "Return to the game library while the next AI challenge becomes available."
              : "Vuelve a la biblioteca mientras preparamos el próximo reto contra la IA."}
          </p>
          <Link
            href="/games"
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-3 font-black hover:bg-violet-500"
          >
            {isEnglish ? "View games" : "Ver juegos"} <ArrowRight size={17} />
          </Link>
        </section>
      </AppLayout>
    );
